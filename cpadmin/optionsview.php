...
if(isset($_GET["delOp"])){
	...
	if($idop!=""){
		$delp["IDOPTION"] = $idop;
		$impcategory_option->delete($delp);
		$delop["id_option"] = $idop;
		$options_tab->delete($delop);
		$images->delete($delp);
	}
  ...
  <div class="PD_main_form1">
	...
	foreach( $optionsList as $key => $value ){
      ...
      <tr>
        ...
        <td align="center" bgcolor="#F2F2F2"><a href="edit_option2.php?idp=<?php echo $_GET['idp'];?>&idop=<?php echo $value->getIDOPTION();?>" style="color:#333333">Edit</a> | <a href="optionsview.php?idp=<?php echo $_GET['idp'];?>&delOp=<?php echo $value->getIDOPTION();?>"  style="color:#333333">Delete</a></strong></td>
      </tr>
      <?php }?>
      </table>
          ...
          <td>Option Name</td><td><input type="text" name="newOption" /></td><td>Second Tab
			 ...
			 $subTabs = $imp_category_tabs4->readArray($params1);
		  					...
		  					?>
          </select><input type="hidden" name="idp" value="<?php echo $_GET['idp'];?>"/></td><td><input type="submit" /></td>
          </tr>
          </table>
          </form>
          </div>
