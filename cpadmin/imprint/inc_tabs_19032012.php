...
//	$childparent=explode("_",$_SESSION["radioButtonSelected"]);
		  ...
		  echo "<table style='width:100%'> 
		  			<tr>
						...
						foreach($listColors as $keys => $valuec){
							...
							echo "<img onclick='imgFilterWithColor(this);' id='".$idoption.'_'.$idcolor.'_'.$idtab."' src='./cpadmin/images/".$options_color->getimages()."' alt='".$options_color->getname()."' style='cursor:pointer;  float: left; clear: none; border: medium none; padding: 0px; margin-right: 2px;width:20px;height:20px;'/>";
						}
	...
	if($optiontype->getnameFields()==1){
	   ...
	   foreach($result as $dt => $data){
		  ...
		  echo "<table style='width:100%'> 
		  			<tr>
		  				<td style='width: 427px;'>Please select your imprinting ".$data["styleName"]."</td>
						<td align='right'>";
